...
{
    public function pcr(){
        $pcrTests = PcrTest::where('registration_type', 'normal')->where('center_id', Auth::user()->rapid_pcr_center_id)->whereDate('sample_collection_date', '>=', Carbon::today())->orderBy('sample_collection_date', 'ASC')->get();
        return view('RapidPCRCenterAdministrator.registered.pcr', compact('pcrTests'));
    }

    public function vaccineDose1(){
        $vaccinations = Vaccination::where('registration_type', 'normal')->where('center_id', Auth::user()->rapid_pcr_center_id)->whereDate('date_of_first_dose', '>=', Carbon::today())->orderBy('date_of_first_dose', 'ASC')->get();
        return view('RapidPCRCenterAdministrator.registered.vaccine-dose-1', compact('vaccinations'));
    }

    public function vaccineDose2(){
        $vaccinations = Vaccination::where('registration_type', 'normal')->where('center_id', Auth::user()->rapid_pcr_center_id)->whereDate('date_of_second_dose', '>=', Carbon::today())->orderBy('date_of_second_dose', 'ASC')->get();
        return view('RapidPCRCenterAdministrator.registered.vaccine-dose-2', compact('vaccinations'));
    }

    public function booster(){
        $boosters = Booster::where('registration_type', 'normal')->where('center_id', Auth::user()->rapid_pcr_center_id)->whereDate('date', '>=', Carbon::today())->orderBy('date', 'ASC')->get();
        return view('RapidPCRCenterAdministrator.registered.booster', compact('boosters'));
    }
